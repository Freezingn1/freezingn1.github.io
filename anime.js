(function () {
    ...
    var ANIME_GENRE_ID = '16'; // ID жанра аниме в TMDB
    var PLUGIN_NAME = 'custom_anime'; // Уникальное имя для нашего плагина
    
    // Стили для нашего раздела
    var customStyles = `
        .${PLUGIN_NAME}-menu-item {
            position: relative;
        }
        .${PLUGIN_NAME}-menu-item .menu__ico {
            background: linear-gradient(135deg, #ff7eb3, #ff758c);
            border-radius: 50%;
            padding: 3px;
        }
        .${PLUGIN_NAME}-menu-item .menu__text {
            color: #ff7eb3;
            font-weight: 500;
        }
        .${PLUGIN_NAME}-submenu {
    ...
    function addMenuButton() {
        ...
        field.on("hover:enter", function () {
            loadAnimeContent({
                ...
                plugin: PLUGIN_NAME // Добавляем метку нашего плагина
            });
        });
        ...
        if ($tvItem.length) {
            $tvItem.after(field);
            setTimeout(function() {
                $(NEW_ITEM_SELECTOR).insertAfter($(ITEM_TV_SELECTOR));
            }, ITEM_MOVE_TIMEOUT);
        }
    }

    // Загрузка контента с уникальным оформлением
    function loadAnimeContent(params) {
        var defaultParams = {
            component: 'category',
            source: 'tmdb',
            page: 1,
            card_type: true,
            plugin: PLUGIN_NAME, // Метка нашего плагина
            card_style: `
                .card[data-plugin="${PLUGIN_NAME}"] {
                    border: 1px solid rgba(255, 126, 179, 0.3);
                }
                .card[data-plugin="${PLUGIN_NAME}"]:hover {
                    border-color: #ff7eb3;
                }
                .card[data-plugin="${PLUGIN_NAME}"] .card__title {
                    color: #ff7eb3 !important;
                }
            `
        };
        
        Lampa.Activity.push(Object.assign(defaultParams, params));
    }

    // Создаем уникальное подменю
    function createSubmenu() {
        var submenuContainer = document.createElement('div');
        submenuContainer.className = `anime-submenu ${PLUGIN_NAME}-submenu`;
        ...
        function toggleSubmenu(show) {
            if (show) {
                ...
                var animeItem = $(`[data-action="${PLUGIN_NAME}"]`)[0];
                if (animeItem) {
        ...
        var menuItems = [
            {
                name: Lampa.Lang.translate('custom_anime_movies'),
                ...
                action: function() {
                    loadAnimeContent({
                        url: 'discover/movie',
                        title: Lampa.Lang.translate('custom_anime_movies'),
                        with_genres: ANIME_GENRE_ID
                    });
                }
            },
            {
                name: Lampa.Lang.translate('custom_anime_tv'),
                ...
                action: function() {
                    loadAnimeContent({
                        url: 'discover/tv',
                        title: Lampa.Lang.translate('custom_anime_tv'),
                        with_genres: ANIME_GENRE_ID
                    });
                }
            },
            {
                name: Lampa.Lang.translate('custom_anime_popular'),
                ...
                action: function() {
                    loadAnimeContent({
                        url: 'discover/tv',
                        title: Lampa.Lang.translate('custom_anime_popular'),
                        with_genres: ANIME_GENRE_ID,
            ...
            {
                name: Lampa.Lang.translate('custom_anime_top'),
                ...
                action: function() {
                    loadAnimeContent({
                        url: 'discover/tv',
                        title: Lampa.Lang.translate('custom_anime_top'),
                        with_genres: ANIME_GENRE_ID,
            ...
            {
                name: Lampa.Lang.translate('custom_anime_new'),
                ...
                action: function() {
                    loadAnimeContent({
                        url: 'discover/tv',
                        title: Lampa.Lang.translate('custom_anime_new'),
                        with_genres: ANIME_GENRE_ID,
            ...
            {
                name: Lampa.Lang.translate('custom_anime_ongoing'),
                ...
                action: function() {
                    loadAnimeContent({
                        url: 'discover/tv',
                        title: Lampa.Lang.translate('custom_anime_ongoing'),
                        with_genres: ANIME_GENRE_ID,
                        with_status: 'returning',
                        sort_by: 'first_air_date.desc'
                    });
                }
            }
        ...
        menuItems.forEach(function(item) {
            ...
            menuItem.className = `selector focusable ${PLUGIN_NAME}-submenu-item`;
        ...
        // Обработчики событий
        $('body').on('hover:enter', `[data-action="${PLUGIN_NAME}"]`, function() {
            toggleSubmenu(true);
        });
    ...
    function setupSettings() {
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                name: Lampa.Lang.translate('custom_anime_title'),
                description: "Показывать кастомный раздел Аниме"
            },
            onChange: function (value) {
                if (value === 'true') {
                    ...
                    $(`[data-action="${PLUGIN_NAME}"]`).remove();
                    $(`.${PLUGIN_NAME}-submenu`).remove();
                }
    ...
    function initPlugin() {
        ...
        var checkMenuInterval = setInterval(function() {
            if (Lampa.Menu.render().length) {
                clearInterval(checkMenuInterval);
                
                // Проверяем настройку и добавляем меню
                if (Lampa.Storage.get(`${PLUGIN_NAME}_enabled`, 'true') === 'true') {
                    addMenuButton();
                    createSubmenu();
                }
            }
    ...
    if (window.Lampa && Lampa.SettingsApi) {
        ...
        document.addEventListener('lampa_loaded', initPlugin);
    }
})();