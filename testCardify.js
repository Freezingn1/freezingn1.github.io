            ...
            this.state = new State({
                ...
                transitions: {
                    start: function start(state) {
                        clearTimeout(_this2.timer_load);
                        if (_this2.player.display) state.dispath('play');
                        else if (_this2.player.loaded) {
                            _this2.animate();
                            _this2.timer_load = setTimeout(function() {
                                state.dispath('load');
                            }, _this2.timelauch);
                        }
                    },
                    load: function load(state) {
                        if (_this2.player.loaded && Lampa.Controller.enabled().name == 'full_start' && _this2.same()) state.dispath('play');
                    },
                    play: function play() {
                        _this2.player.play();
                    },
                    toggle: function toggle(state) {
                        clearTimeout(_this2.timer_load);
                        if (Lampa.Controller.enabled().name == 'cardify_trailer'); else if (Lampa.Controller.enabled().name == 'full_start' && _this2.same()) {
                            state.start();
                        } else if (_this2.player.display) {
                            state.dispath('hide');
                        }
                    },
                    hide: function hide() {
                        _this2.player.pause();
                        _this2.player.hide();
                        _this2.background.removeClass('nodisplay');
                        _this2.startblock.removeClass('nodisplay');
                        _this2.head.removeClass('nodisplay');
                        _this2.object.activity.render().find('.cardify-preview__loader').width(0);
                    }
                }
            });
            this.start();
        ...
        _createClass(Trailer, [{
            ...
            value: function animate() {
                ...
                this.timer_anim = setInterval(function() {
                    ...
                    if (left > _this3.timelauch) clearInterval(_this3.timer_anim);
                    loader.width(Math.round(left / _this3.timelauch * 100) + '%');
                }, 100);
            }
        }, {
            key: "preview",
            ...
            value: function start() {
                ...
                this.player.listener.follow('play', function() {
                    ...
                    _this5.timer_show = setTimeout(function() {
                        _this5.player.show();
                        _this5.background.addClass('nodisplay');
                        _this5.startblock.addClass('nodisplay');
                        _this5.head.addClass('nodisplay');
                        _this5.controll();
                    }, 500);
                });
