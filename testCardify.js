(function () {
  ...
  function displayLogo(element, data) {
    if (data.images && data.images.logos && data.images.logos.length > 0) {
      // Ищем русский логотип
      let ruLogo = data.images.logos.find(logo => logo.iso_639_1 === 'ru');
      // Если нет русского, ищем английский
      let enLogo = !ruLogo ? data.images.logos.find(logo => logo.iso_639_1 === 'en') : null;
      // Используем первый доступный логотип, если нет ни русского, ни английского
      let logo = ruLogo || enLogo || data.images.logos[0];
      
      if (logo) {
        let img = new Image();
        img.src = 'https://image.tmdb.org/t/p/original' + logo.file_path;
        img.onload = function() {
          element.innerHTML = '';
        ...
        img.onerror = function() {
          element.textContent = data.title || data.name;
        };
      } else {
        element.textContent = data.title || data.name;
      }
  ...
  var Player = /*#__PURE__*/function () {
    function Player(object, video) {
      ...
      // Отображаем логотип для трейлера
      setTimeout(() => {
        if (object.data) {
          let titleElement = this.html.find('.cardify-trailer__title')[0];
          displayLogo(titleElement, object.data);
        }
      }, 100);
  ...
  function startPlugin() {
    ...
    Lampa.Listener.follow('full', function (e) {
      if (e.type == 'complete') {
        ...
        // Отображаем логотип для основного заголовка
        let titleElement = e.object.activity.render().find('.full-start-new__title')[0];
        if (titleElement) {
          displayLogo(titleElement, e.data);
        }
