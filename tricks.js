(function () {
    ...
    function add() {
        // Сначала создаем компонент для меню настроек
        Lampa.Component.add('tweaks_menu', {
            template: `
                <div class="settings-other">
                    <div class="settings-other__scroll">
                        <div class="settings-other__items"></div>
                    </div>
                </div>
            `,
            mounted: function() {
            ...
            methods: {
                renderParams: function() {
                    let params = Lampa.SettingsApi.params('tweaks_menu');
                    let html = '';

                    params.forEach(param => {
                        html += `
                            <div class="settings-other__item selector">
                                <div class="settings-other__item__name">${param.field.name}</div>
                                <div class="settings-other__item__value">${this.getParamValue(param)}</div>
                            </div>
                        `;
                    });

                    this.$el.find('.settings-other__items').html(html);
                },
                getParamValue: function(param) {
                    if (param.param.type === 'toggle') {
                        return `<div class="switch ${param.value ? 'active' : ''}">
                            <div class="switch__box"></div>
                        </div>`;
                    }
                    return param.value;
                }
        ...
        // Затем добавляем меню в настройки
        Lampa.SettingsApi.addComponent({
            component: 'tweaks_menu',
            name: 'Tweaks & Tricks',
        ...
        // Добавляем параметр для кнопки перезагрузки
        Lampa.SettingsApi.addParam({
            ...
            onChange: function(value) {
                Lampa.Storage.set('tweaks_reloadbutton', value);
                updateReloadButton();
            }
        });
        ...
        function updateReloadButton() {
            const enabled = Lampa.Storage.get('tweaks_reloadbutton', true);
            if (enabled) {
        ...
        const reloadButton = `
            <div id="RELOAD" class="head__action selector reload-screen ${Lampa.Storage.get('tweaks_reloadbutton', true) ? '' : 'hide'}">
        ...
        $('div.head__actions').append(reloadButton);
