...
function getStoredSetting(key, defaultValue) {
    // Проверяем, относится ли ключ к кастомным кнопкам
    if (key.startsWith('custom_button_')) {
        // Получаем ID кнопки из ключа
        var buttonId = key.replace('custom_button_', '');
        // Находим кнопку в списке
        var button = getAllButtons().find(function(b) { return b.id === buttonId; });
        // Если кнопка найдена и у нее есть свойство defaultEnabled, используем его
        if (button && button.hasOwnProperty('defaultEnabled')) {
            defaultValue = button.defaultEnabled;
        } else {
            // По умолчанию для всех кнопок - false
            defaultValue = false;
        }
    }
	
	 // Специальные значения по умолчанию для некоторых параметров
    var defaultValues = {
        'show_popular_persons': false,
        'custom_button_surs_main': false,
        'custom_button_surs_bookmarks': false,
        'custom_button_surs_history': false,
        'custom_button_surs_select': false,
        'custom_button_surs_new': false,
        'custom_button_surs_rus': false,
        'custom_button_surs_kids': false
    };
    
    // Если ключ есть в defaultValues, используем соответствующее значение по умолчанию
...
if (getStoredSetting('show_popular_persons', false)) { // По умолчанию false
...
function addSettingMenu() {
    ...
    field: {
        name: Lampa.Lang.translate('surs_show_popular_persons'),
        description: Lampa.Lang.translate('surs_show_popular_persons_desc')
    },
    onChange: function() {
        var currentController = Lampa.Controller.enabled().name;
        Lampa.Select.show({
...
surs_show_popular_persons: {
    ...
    surs_show_popular_persons_desc: {
        ru: "Показывать раздел с популярными актерами и режиссерами",
        en: "Show section with popular actors and directors",
        uk: "Показувати розділ з популярними акторами та режисерами"
    },
    surs_enabled: {
        ru: "Включено",
        en: "Enabled",
