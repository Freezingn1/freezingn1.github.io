function add() {
    // Удаляем зависимость от SourceTMDB классов
    var tmdb_mod = Lampa.Api.sources.tmdb;
    var tmdb_mod_kids = Lampa.Api.sources.tmdb;
    var tmdb_mod_rus = Lampa.Api.sources.tmdb;
    ...
    // Регистрируем LNUM как источник
    (function () {
        // Polyfills
        ...
        function LNumApiService() {
            ...
            // ... (остальные методы класса LNumApiService остаются без изменений)
            // Вставьте сюда все остальные методы из оригинального кода LNumApiService
            // включая getCache, setCache, normalizeData, getFromCache, get, list, full, category
            // и вспомогательные функции getBaseUrl и getToken
                ...
                if (COLLECTIONS.length === 0) {
                    self.network.silent(LNUM_COLLECTIONS_BASE_URL + '?session_id=' + SESSION_ID + '&lnum_token=' + LNUM_TOKEN, function (json) {
                        if (json.success) {
                            ...
                            console.log('Loaded collections in category:', COLLECTIONS);
        ...
        function Episode(data) {
            // ... (оставьте реализацию класса Episode без изменений)
            var self = this;
...
// Вызываем функцию для добавления плагина
add();