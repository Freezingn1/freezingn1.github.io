(function () {
    ...
    function cancelPendingRequests() {
        Object.keys(activeRequests).forEach(url => {
    ...
    function create() {
        ...
        this.update = function (data) {
            ...
            // Обновляем логотип или заголовок
        ...
        this.background = function (data) {
            ...
            const new_background = data.backdrop_path 
            ...
            background_timer = setTimeout(() => {
                const bgImg = html.find('.full-start__background');
                if (!bgImg.length) return;

                bgImg.css('opacity', 0);
                bgImg[0].onload = function() {
                    bgImg.css('opacity', 1);
                };
        ...
        this.loadLogo = function(data) {
            ...
            const request = network.silent(url, (images) => {
                if (isDestroyed || !html) return;
                delete activeRequests[url];
                
                const bestLogo = this.findBestLogo(images.logos);
        ...
        this.findBestLogo = function(logos) {
            ...
            logos.forEach(logo => {
                let score = logo.vote_average || 0;
                // Предпочтение русским и английским логотипам
                if (logo.iso_639_1 === 'ru') score += 2;
                else if (logo.iso_639_1 === 'en') score += 1;
        ...
        this.applyLogo = function(data, logo) {
            ...
            // Если этот логотип уже отображается, ничего не делаем
            ...
            logo_timer = setTimeout(() => {
                ...
                if (logoCache[imageUrl]) {
                    titleElement.html(logoCache[imageUrl]);
                    setTimeout(() => {
                ...
                const logoHtml = `
                    <img class="new-interface-logo" 
                         ...
                         onerror="this.style.display='none'; this.parentElement.textContent='${(data.title || data.name).replace(/"/g, '&quot;')}'" />
                `;

                addToCache(logoCache, imageUrl, logoHtml);
    ...
    function component(object) {
        ...
        var items = [];
        var html = $('<div class="new-interface"><img class="full-start__background" style="opacity: 0; transition: opacity 0.3s ease;"></div>');
        var active = 0;
