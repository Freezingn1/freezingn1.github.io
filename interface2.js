(function () {
    ...
    function applyPlatformScreenOverride(orientation) {
        ...
        if (orientation === 'vertical') {
            ...
            if (platformScreenOverride) {
                Lampa.Platform.screen = originalPlatformScreen;
    ...
    function create() {
        ...
        var logo_timer;
        var titleElement, headElement, detailsElement;
            ...
            if (logoSetting === 'logo_on') {
                ...
                network.silent(url, (images) => {
                    ...
                    if (images.logos && images.logos.length > 0) {
                        ...
                        // Оптимизированный поиск лучшего логотипа
                        for (var i = 0; i < images.logos.length; i++) {
                            var logo = images.logos[i];
                    ...
                    if (!isDestroyed && html && titleElement) {
                        titleElement.text(data.title);
                    }
                });
            } else if (!isDestroyed && html && titleElement) {
                titleElement.text(data.title);
            }
        ...
        this.applyLogo = function(data, logo) {
            if (isDestroyed || !html || !titleElement) return;
        ...
        this.draw = function (data) {
            if (isDestroyed || !html || !headElement || !detailsElement) return;
