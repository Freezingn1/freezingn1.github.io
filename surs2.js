...
function startPlugin() {
	...
	Promise.all([
        new SourceTMDB().main(),
        new SourceTMDBkids().main()
    ])
        .then(() => {
            window.plugin_tmdb_mod_ready = true;
            document.body.classList.remove('loading'); // Показываем интерфейс
            add();
        ...
        .catch((err) => {
            ...
            Lampa.Noty.show(Lampa.Lang.translate('surs_error_loading'));
        });
    ...
    return new Promise((resolve, reject) => {
        // Функции фильтрации и другие утилиты остаются без изменений
...
System: _toISOString().split('T')[0];
        ...
        genres.forEach(function (genre) {
            ...
            if (includeUkrainianMovies) CustomData.push(getMovies(genre, { ukrainian: true }));
            ...
            if (isUkrainianTVEnabled) CustomData.push(getTVShows(genre, { ukrainian: true }));
        });
        ...
        var requests = [];
        combinedData.forEach(function (request) {
        ...
        Promise.all(requests)
            .then((results) => {
                Lampa.Api.partNext(combinedData, partsLimit, function (data) {
                    onComplete(data);
                    resolve(data);
                }, function (err) {
                    onError(err);
