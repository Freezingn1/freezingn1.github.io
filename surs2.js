...
function getWithCache(url, params, callback) {
    var cacheKey = JSON.stringify({url: url, params: params});
    ...
    owner.get(url, params, function(json) {
        requestCache[cacheKey] = json;
        callback(json);
    }, callback);
}

var currentPage = 1;
var itemsPerPage = 5; // Количество подборок на "странице"
...
function loadNextPage() {
    ...
    Lampa.Api.partNext(pageData, itemsPerPage, function() {
        currentPage++;
        // Загрузить следующую страницу при прокрутке
    }, partEmpty);
}
